...
async function main() {
  ...
  if (argv.includes("--abort")) {
    ...
    console.log("Rebase aborted. You can try cleanup-worktree again or continue working.");
    process.exit(0);
  }
  ...
  const untrackedFiles = (await $`git ls-files --others --exclude-standard`.text()).trim();
  if (untrackedFiles) {
    console.error("Error: you have untracked files");
    console.error("Please commit, remove, or add them to .gitignore before cleaning up");
    process.exit(1);
  }
  ...
  const commitCountOutput = await $`git rev-list --count ${mainBranch}..${currentBranch}`.nothrow().quiet();
  const commitCount = parseInt(commitCountOutput.stdout.toString().trim()) || 0;

  if (commitCount === 0) {
    console.log(`No commits to rebase (branch is up to date with ${mainBranch})`);
  } else {
  ...
  const logOutput = await $`git log --oneline ${mainBranch}..${currentBranch}`.nothrow().quiet();
